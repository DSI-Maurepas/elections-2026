    ...
    <div className="passage-t2">
      ...
      <div className="actions">
        <button
          ...
          disabled={passageConfirme || !secondTourEnabled || loading || egalite || candidatsTete.length !== 2}
          className="btn-primary"
          title={passageConfirme ? "Second tour actif (retour possible via Administration)" : (!secondTourEnabled ? "Désactivé : activer via Administration" : undefined)}
        >
          {loading
            ? 'Traitement...'
            : !secondTourEnabled
              ? '⛔ Passage au 2nd tour désactivé'
              : passageConfirme ? '✅ Second tour actif' : '➡️ Confirmer passage au 2nd tour'}
        </button>
      </div>
    </div>
  );
