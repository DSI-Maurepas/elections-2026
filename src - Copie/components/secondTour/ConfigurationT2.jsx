      ...
      >
        ...
        {!state?.candidatsQualifies || state.candidatsQualifies.length !== 2 ? (
          ...
          <div className="message success" style={{ marginTop: '0.9rem' }}>
            ✅ Passage au 2nd tour confirmé : <strong>{state.candidatsQualifies?.[0]?.nom || state.candidatsQualifies?.[0]?.nomListe}</strong> et{' '}
            <strong>{state.candidatsQualifies?.[1]?.nom || state.candidatsQualifies?.[1]?.nomListe}</strong>
          </div>
        )}
      </div>
    </div>
