...
const PassageSecondTour = () => {
  ...
  const [showT2CertifiedModal, setShowT2CertifiedModal] = useState(false);
  const [localTourActuel, setLocalTourActuel] = useState(null);

  const tourActuel = useMemo(() => {
    const local = Number(localTourActuel);
    if (local === 1 || local === 2) return local;
    const raw = electionState?.tourActuel ?? electionState?.currentTour ?? electionState?.tour ?? 1;
    return Number(raw) === 2 ? 2 : 1;
  }, [electionState, localTourActuel]);

  useEffect(() => {
    // Dès que le state global reflète le tour, on relâche l'override local
    const raw = electionState?.tourActuel ?? electionState?.currentTour ?? electionState?.tour;
    if (raw == null) return;
    const t = Number(raw) === 2 ? 2 : 1;
    if (t === 1 || t === 2) setLocalTourActuel(null);
  }, [electionState]);
  ...
  const handlePassageT2 = async () => {
    ...
    if (
      ...
      try {
        ...
        // Double certification : UI immédiate + modal plein écran
        setLocalTourActuel(2);
        setShowT2CertifiedModal(true);
        setMessage({
          type: 't2',
          text: '🔵 Vous êtes maintenant dans le second tour',
        });
      } catch (error) {
        setMessage({
          type: 'error',
    ...
    <div className="passage-t2">
      ...
      <div className="actions">
        <button
          ...
          disabled={tourActuel === 2 || !secondTourEnabled || loading || egalite || candidatsQualifies.length !== 2}
          className={tourActuel === 2 ? 'btn-primary btn-t2' : 'btn-primary'}
          title={!secondTourEnabled ? "Désactivé : activer via Administration" : undefined}
        >
          {loading
            ? 'Traitement...'
            : tourActuel === 2
              ? '🔵 Vous êtes maintenant dans le second tour'
              : !secondTourEnabled
      ...
      {/* Modal plein écran : certification officielle du passage au 2nd tour */}
      {showT2CertifiedModal && (
        <div
          role="dialog"
          aria-modal="true"
          style={styles.fullscreenOverlay}
          onClick={() => setShowT2CertifiedModal(false)}
        >
          <div
          ...
          >
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12 }}>
              <h3 style={{ margin: 0, color: '#1e40af' }}>🔵 Passage au 2nd tour confirmé</h3>
              <button
            ...
            <p style={{ marginTop: 10, marginBottom: 0, fontSize: 16 }}>
              Vous êtes maintenant dans le <strong>second tour</strong>.
            </p>
            <p style={{ marginTop: 8, opacity: 0.85 }}>
              La charte doit apparaître en bleu et les saisies/résultats concernent désormais le tour 2.
            </p>
          </div>
        </div>
      )}
