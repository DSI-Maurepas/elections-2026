...
import authService, { isAdmin } from '../../services/authService';
...
const PassageSecondTour = ({
  ...
  return (
    <>
      ...
      {showConfirmBackT1Modal && (
        <div style={styles.modalOverlay} role="dialog" aria-modal="true">
          <div style={styles.modal}>
            <h3 style={styles.modalTitle}>Confirmer le retour au 1er tour</h3>
            <p style={{ marginTop: 8 }}>
              Cette action revient au <strong>1er tour</strong> et déverrouille le tour. Elle est réservée à l’administration.
            </p>
            <div style={styles.modalActions}>
              <button
                type="button"
                className="btn btn-secondary"
                onClick={() => setShowConfirmBackT1Modal(false)}
                disabled={loading}
              >
                Annuler
              </button>
              <button
                ...
                onClick={async () => {
                  try {
                    ...
                    setShowConfirmBackT1Modal(false);
                    await reloadElectionState?.();
                    setMessage({ type: 'success', text: 'Retour au 1er tour effectué.' });
                  } catch (e) {
                    setMessage({ type: 'error', text: e?.message || 'Erreur lors du retour au 1er tour.' });
                  } finally {
                    setLoading(false);
                  }
                }}
